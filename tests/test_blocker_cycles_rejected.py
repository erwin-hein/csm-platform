...
def test_blockers_reject_self_cycles_and_cross_engagement(db, world):
    ...
    a, b, c = (D.create_deliverable(db, world.admin, eid, kind="phase", name=n) for n in "abc")
...
def test_blocked_flag_requires_reason(db, world):
    p = D.create_deliverable(db, world.admin, world.alice_eng.id, kind="phase", name="p")
