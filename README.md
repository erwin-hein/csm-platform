...
6. **The client side.** On *Bluefin Tableau → Omni Migration*, the **Client view** tab shows exactly what the client's project lead sees: dashboards and tiles only, never phases or milestones. The team panel lists the two client users and their scope. In any tile's 💬 pop-up, *Internal notes* and the *Client thread* are separate tabs with separate colours.
